...
export default function DialogTaskChange(props) {
  const {open, handleClose, handleChangeDoing, taskChangeDoing} = props;
  ...
  const handleTask = () => {
    handleChangeDoing({...task});
    setTask({});
  }

  useEffect(() => {
    console.log(taskChangeDoing)
    if(taskChangeDoing)setTask(taskChangeDoing)
  }, [taskChangeDoing])
